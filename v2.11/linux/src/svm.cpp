...
#define INF 1e10//HUGE_VAL	//Modificado por Modesto
#define TAU 1e-12
#define Malloc(type,n) (type *)malloc((n)*sizeof(type))
#if 0 //1 Modificado por Modesto ya que no hay salida estándard
void info(char *fmt,...)
{
	va_list ap;
	va_start(ap,fmt);
...
//Añadido por Modesto
double svm_predictV(const svm_model *model, const svm_node *x, int* classvote)
{
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR)
